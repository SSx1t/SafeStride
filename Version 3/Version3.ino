/*
 * SafeStride VERSION 3 — ESP32 firmware (MYOSA platform)
 *
 * What changed in V3 (2026-05-16):
 *  - New ML model: rf_coral_F.pkl (CORAL domain-adapted RF) embedded as
 *    gait_model.h. Trained on Felius healthy + Felius stroke + our healthy,
 *    with Felius features mapped into our hardware's feature distribution
 *    via CORAL. Cross-subject CV: 86.9% acc, 90.5% stroke recall.
 *  - Step-peak threshold: 0.35*G → 0.20*G (Felius-tuned threshold was missing
 *    most footstrikes on our hardware; cadence was reporting at half its
 *    true value).
 *  - Autocorr lag range: 30-80 → 30-200 samples (covers slow walking down
 *    to 30 spm; old range only handled 75-200 spm).
 *  - isValidGaitWindow cadence floor relaxed to match (30-200 spm).
 *  - SERIAL_DEBUG flag added (false by default) so no UART work on hot path
 *    when nothing is listening over USB.
 *  - All ML processing happens here on the ESP. The phone app only displays
 *    the verdicts received via the ML BLE characteristic (UUID …5b1a05).
 *
...
public:
  ...
  // V3: expose recent-window counts so the app can render a confidence
  // (e.g. 7/7 stroke = high confidence stroke; 4 healthy 3 stroke = wobble).
  int healthyCount() const {
    int n = 0; for (int i = 0; i < count; i++) if (hist[i] == LABEL_HEALTHY_GAIT) n++;
    return n;
  }
  int strokeCount() const {
    int n = 0; for (int i = 0; i < count; i++) if (hist[i] == LABEL_STROKE_GAIT) n++;
    return n;
  }
...
int findStepPeaks(const float* x, int n, int* peakIdx, int maxPeaks) {
  // V3: lowered 0.35→0.20*G. Felius-tuned 0.35 missed most footstrikes on
  // our MYOSA+filter chain (only ~1.7% of samples exceeded), making cadence
  // report ~half its true value. 0.20*G matches spectral cadence on this hw.
  const float threshold = 0.20f * G_CONST;
...
bool extractGaitFeatures(const GaitFeatureWindow& win, float features[18]) {
  ...
  // V3: lag range extended from 30-80 to 30-200 samples. Old range covered
  // only 75-200 spm; slow walking at 30-75 spm couldn't be detected by the
  // autocorrelation peak search. 30-200 covers the full human gait envelope.
  float bestAc = 0.0f;
  int bestLag = 0;
  for (int lag = 30; lag < 200; lag++) {
...
bool isValidGaitWindow(const float features[18]) {
  // V3: relaxed cadence floor to match the extended autocorr range so slow
  // walking (down to ~30 spm) is still classified rather than gated out as
  // "no_gait". Upper bound nudged to 200 to match the lag search.
  bool enoughSteps = features[10] >= 4.0f;
  bool cadenceOk = features[11] >= 30.0f && features[11] <= 200.0f;
...
void publishMlLabel(MlLabel rawLabel, MlLabel stableLabel, const float features[18]) {
  // V3 wire format (single ASCII line, comma separated):
  //   <tMs>,<raw>,<stable>,<stepCount>,<cadence>,<stepAc>,<healthyN>,<strokeN>,<validN>
  // Where:
  //   raw / stable ∈ "no_gait" | "healthy_gait" | "stroke_gait"
  //   stepCount, cadence, stepAc — features [10], [11], [13] for display.
  //   healthyN, strokeN, validN — smoother counts so the phone can derive
  //                               a "healthy fraction" confidence (h / (h+s)).
  char msg[200];
  unsigned long t = millis() - recordStart;
  snprintf(msg, sizeof(msg), "%lu,%s,%s,%.0f,%.1f,%.3f,%d,%d,%d",
           t,
           ...
           features[11],
           features[13],
           mlSmoother.healthyCount(),
           mlSmoother.strokeCount(),
           mlSmoother.validCount());
  if (mlChar != nullptr) {
